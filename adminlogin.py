...
class Login_System:
    def __init__(self, root):
        ...
        title = Label(login_frame, text="Admin Log In", font=("Elephant", 30, "bold"), bg='white').place(x=0, y=30,
                                                                                                         relwidth=1)

        lbl_user = Label(login_frame, text="Admin Username", font=("Andalus", 15), bg="white", fg="#767171").place(x=50,
                                                                                                                y=100)
        ...
        btn_forget = Button(login_frame, text="Not Admin? Go back to sign in page", command=self.notadmin,
                            font=("times new roman", 13), bg="white", fg="blue", bd=0, activebackground="white",
                            activeforeground="blue", cursor="hand2").place(x=50, y=390)
    ...
    def login(self):
        ...
        try:
            if self.employee_id.get() == "" or self.password.get() == "":
                ...
                cur.execute("select username from adminlogin where username=%s AND password=%s",
