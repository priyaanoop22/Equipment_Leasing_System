...
class adminClass:
    def __init__(self, root):
        ...
        # title
        title = Label(self.root, text="Admin Approval Page", font=("times new roman", 15), bg="#0f4d7d", fg="white").pack(side=TOP,fill=X)
        ...
        # Buttons
        btn_add = Button(self.root, text="Approve", command=self.add, font=("goudy old style", 15), bg="#2196f3",
                         fg="white", cursor="hand2").place(x=500, y=305, width=110, height=28)
        btn_update = Button(self.root, text="Approved List", command=self.show2, font=("goudy old style", 13), bg="#4caf50",
                            fg="white", cursor="hand2").place(x=740, y=305, width=110, height=28)
        btn_delete = Button(self.root, text="Reject", command=self.delete, font=("goudy old style", 15), bg="#f44336",
        ...
        btn_pending = Button(self.root, text="Pending List", command=self.show, font=("goudy old style", 13), bg="#a334ac",
                           fg="white", cursor="hand2").place(x=980, y=305, width=110, height=28)
        ...
        try:
            if self.var_emp_id.get() == "":
                ...
                if row != None:
                    ...
                    con.commit()
                    cur.execute("delete from pending where eid=%s", (self.var_emp_id.get(),))
                    con.commit()
                    messagebox.showinfo("Success", "Employee Approved Successfully", parent=self.root)
                    self.show2()
    ...
    def delete(self):
        ...
        try:
            if self.var_emp_id.get() == "":
                ...
                cur.execute("Select * from pending where eid=%s", (self.var_emp_id.get(),))
                row = cur.fetchone()
                if row == None:
                    messagebox.showerror("Error", "Invalid Employee ID", parent=self.root)
                else:
                    op = messagebox.askyesno("Confirm", "Do you really want to reject?", parent=self.root)
                    if op == True:
                        cur.execute("delete from pending where eid=%s", (self.var_emp_id.get(),))
                        con.commit()
                        messagebox.showinfo("Reject", "Employee Rejected Successfully", parent=self.root)
                        self.clear()
