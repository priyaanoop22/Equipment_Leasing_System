...
class categoryClass:
    def __init__(self, root):
        ...
        # Title
        lbl_title = Label(self.root, text="Manage Equipment Category", font=("times new roman", 30), bg="#184a45",
                          fg="white", bd=3, relief=RIDGE).pack(side=TOP, fill=X, padx=10, pady=2)

        lbl_title = Label(self.root, text="Enter Category Name", font=("times new roman", 30), bg="white").place(x=50,
                                                                                                                 y=100)
        txt_name = Entry(self.root, textvariable=self.var_name, font=("times new roman", 19), bg="lightgreen").place(
            x=50, y=170, width=300)

        btn_add = Button(self.root, text="ADD", command=self.add, font=("times new roman", 15), bg="purple", fg="white",
        ...
        self.im1 = self.im1.resize((500, 350), Image.ANTIALIAS)
        ...
        self.im2 = self.im2.resize((500, 350), Image.ANTIALIAS)
        self.im2 = ImageTk.PhotoImage(self.im2)

        self.lbl_im2 = Label(self.root, image=self.im2, bd=2, relief=RAISED)
        self.lbl_im2.place(x=575, y=220)
    ...
    def delete(self):
        ...
        try:
            if self.var_cat_id.get() == "":
                ...
                row = cur.fetchone()
                if row == None:
                    messagebox.showerror("Error", "Try Again Plz.", parent=self.root)
                else:
